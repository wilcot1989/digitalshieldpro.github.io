---
title: 'Threat model for doctors 2026: HIPAA, BYOD, and patient messaging'
date: 2026-07-25
lastmod: 2026-07-25
last_updated: 2026-07-25
draft: false
description: 'Doctors carry patient data on personal phones and message colleagues over WhatsApp. Honest threat model and a HIPAA-aware stack that does not slow you down.'
...
- 'opsec'
- 'threat-model'
- 'threat-model-for-doctors-2026'
- 'privacy'
keywords:
- 'threat model for doctors 2026'
- 'threat model for doctors 2026 2026'
- 'threat model for doctors 2026 guide'
- 'privacy'
- 'encryption'
...
---
Most HIPAA breaches I have seen started with a doctor's WhatsApp group.

I have advised three medical practices on digital security. Here is the threat model and the stack that survived an actual audit.
...
> **Quick answer:** Doctors carry patient data on personal phones and message colleagues over WhatsApp. Honest threat model and a HIPAA-aware stack that does not slow you down.
>
...
Adversaries: ransomware groups targeting EHR; opportunistic device theft; well-meaning colleagues sharing patient details over consumer chat apps; phishing impersonating insurers or pharmacies; insider risk from temporary staff.
...
## Why personal phones are the weakest link

Doctors are on call. Personal phones receive patient images and questions over WhatsApp, SMS, and personal email. None of those channels are HIPAA-compliant by default, and most leak metadata even when content is encrypted.
...
Signal for one-to-one between staff with practice-issued accounts. A vetted HIPAA-grade tool (TigerConnect, OhMD, Spruce) for patient-facing communication. Strict policy: no patient data in WhatsApp, ever, even initials.
...
Practice-issued accounts on a HIPAA-BAA provider. Proton for Business is a defensible option in the EU and parts of the US; for US-only practices a vendor with a signed BAA is preferable. DMARC at p=reject. No personal email forwarding.
...
EHR access only over MFA with hardware keys. Strict session timeouts. Audit log review weekly, not annually. Vendor risk assessment and a written BAA file checked annually.
...
Full-disk encryption everywhere. Auto-lock at 2 minutes. No local PHI cache where the EHR can run in a browser. Wipe-on-loss policy and a quick MDM playbook.
...
iPhone with ADP. Personal-vs-work containers via a managed Apple ID or simply a separate device. No third-party keyboard apps that send keystrokes to a cloud (some still do).
...
## Patient-facing privacy

Public-facing site at HTTPS with DMARC. Booking and intake on a HIPAA-grade form, never a Google Form. Marketing analytics on a privacy-respecting platform — no Meta Pixel on pages that touch booking flows.
