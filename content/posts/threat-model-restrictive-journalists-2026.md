---
title: 'Threat model: journalists in restrictive countries 2026'
date: 2026-07-29
lastmod: 2026-07-29
last_updated: 2026-07-29
draft: false
description: 'Working journalists in restrictive countries face hostile devices, networks, and contacts. Field-tested threat model and a stack that survives airports.'
...
---
Operational security for journalists in restrictive countries is the only threat model where mistakes are physical.

I have helped journalists prepare for assignments in three countries where the press faces criminal charges. Here is the threat model and the stack that has not failed.
...
> **Quick answer:** Working journalists in restrictive countries face hostile devices, networks, and contacts. Field-tested threat model and a stack that survives airports.
>
...
State-level adversaries with full network observation, lawful device seizure, source-protection laws that do not apply, and an interest in compromising contacts as much as the journalist. Capabilities exceed any consumer threat model.
...
## Device hygiene before travel

Travel laptop and travel phone provisioned for the trip. No historical email, no past sources, no production keys. Bind device encryption keys to a hardware token that does not travel with the device.
...
Compelled disclosure of device passwords is legal in many jurisdictions. The defense is having nothing on the device worth disclosing. Cloud-stored, end-to-end encrypted material is fetched after entry over a clean tunnel.
...
Tor over a bridge that is not in the public list. A backup of WireGuard with obfuscation (AmneziaWG, Cloak). A second backup of OpenVPN over TLS to port 443. Test all three before departure, every trip.
...
Sources are contacted through Signal with disappearing messages, never directly from the journalist's primary number. Use a SIM purchased on arrival in a country where that is legal. Dedicated burner-only contact list.
...
Working drafts on an encrypted volume on the laptop. Daily encrypted upload to a Proton Drive or a self-hosted Nextcloud over the tunnel. Local copy wiped if the device is seized.
...
## Communications back to the desk

Proton Mail for non-urgent dispatch. SecureDrop where the publication runs one. Signal for urgent coordination with the editor. Voice and video only over Signal — never over country-resident telecom.
...
Encrypted backup of the trip uploaded before crossing the border out. Travel devices wiped or destroyed depending on what they touched. Sources notified on a secure channel that the trip is over and the journalist is out.
