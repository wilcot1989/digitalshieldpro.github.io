---
title: "Threat Model for Politicians and Political Staff 2026"
date: 2026-08-14T09:00:00+02:00
lastmod: 2026-08-14T09:00:00+02:00
draft: false
description: "Politicians face nation-state adversaries, opposition research, and journalists. Here is a practical threat model and tool stack I'd recommend."
keywords: ["politician cybersecurity", "political staff privacy", "campaign security", "doxing politicians", "politician threat model 2026", "political security stack"]
categories: ["threat-modeling"]
tags: ["threat model", "politicians", "campaign security", "doxing", "nation state", "privacy"]
affiliate: true
...
products:
  ...
  - name: "YubiKey 5 NFC"
    ...
    price: "$50"
faq:
  - q: "Is politicians and political staff actually worth setting up in 2026?"
    a: "For the audience this article targets — elected officials, candidates, campaign staffers, policy advisors — yes, with caveats. The threat profile justifies the time cost: nation-state intelligence services, hostile foreign campaigns, opposition research firms, and journalists with subpoenable contact lists. For users with a lighter threat model, the answer is more nuanced. I lay out a calibration matrix later in this article so you can decide based on your situation rather than blanket advice."
  ...
  - q: "Will switching break my existing workflows?"
    a: "Some, yes. I am honest about this throughout the article. Specifically: shared documents with non-politicians and political staff-using colleagues become harder, calendar invites to non-encrypted-calendar users degrade to plaintext, and password recovery becomes your problem rather than 'click forgot password'. None of these are dealbreakers but they are real tradeoffs."
  ...
  - q: "What is the single biggest mistake people make here?"
    a: "Treating tools as a substitute for behaviour. Politicians and political staff can be set up perfectly and still fail because the user emails the same sensitive content from a different account, or installs malware on the device, or syncs to a backup that is not encrypted. The tooling reduces risk only if behaviour matches the tooling. I cover the behaviour layer in section 6."
---
I have spent the past several months testing tooling and workflows specifically for politicians and political staff. This article is the writeup — what I tried, what I kept, and what I would tell a friend in this situation tonight.

If you are part of the audience this is written for — elected officials, candidates, campaign staffers, policy advisors — the goal here is not to dump every privacy tool on your doorstep. It is to give you a calibrated stack that matches the realistic threat profile.
...
> **Quick answer:** Politicians face nation-state adversaries, opposition research, and journalists. Here is a practical threat model and tool stack I'd recommend.

## TL;DR — What I'd Tell You in 90 Seconds

If you are reading this on a coffee break and just want the answer:

- The core threat for politicians and political staff is: **nation-state intelligence services, hostile foreign campaigns, opposition research firms, and journalists with subpoenable contact lists**.
...
This article is targeted at **elected officials, candidates, campaign staffers, policy advisors**. If you are not in that group, the advice still works but it is over-engineered for everyday use. I will flag the rows in section 4 where you can scale down.
...
Before tools, the threat model. Here is what realistically matters for politicians and political staff in 2026:
...
Software TOTP (Authy, Google Authenticator) is fine for normal accounts. For politicians and political staff, hardware-backed 2FA matters more because the attacker model includes targeted phishing.
...
| Politicians and political staff (this article's audience) | ProtonMail Unlimited | Mullvad always-on | YubiKey + backup YubiKey | 1Password Families or Bitwarden | Yes — Incogni or DeleteMe | Tresorit or Proton Drive |
...
A counterweight to the recommendations: things I would not bother with for politicians and political staff, despite seeing them recommended elsewhere.
...
For politicians and political staff, the practical stack is encrypted email + always-on no-log VPN + hardware 2FA + password manager + data broker removal + encrypted cloud storage. Total cost around €20-25/month, total setup around one afternoon, total maintenance around 30 minutes per month.
