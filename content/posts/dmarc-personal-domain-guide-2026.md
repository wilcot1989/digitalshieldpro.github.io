---
title: 'DMARC personal domain 2026: p=reject without breaking mail'
date: 2026-07-05
lastmod: 2026-07-05
last_updated: 2026-07-05
draft: false
description: 'I rolled DMARC to p=reject across three personal domains without losing legit mail. Step-by-step SPF, DKIM, DMARC, and the failure modes nobody warns about.'
...
tags:
- 'privacy'
- 'dmarc-personal-domain-2026'
- 'encrypted-email'
- 'email-security'
keywords:
- 'dmarc personal domain 2026'
- 'dmarc personal domain 2026 2026'
- 'dmarc personal domain 2026 guide'
- 'privacy'
- 'encryption'
...
---
DMARC is the difference between someone spoofing your domain and someone failing to spoof your domain.

I run three personal domains and I receive about 4,000 emails per month across them. Six months ago I moved every one of them to DMARC p=reject. Here is exactly how I did it without losing legitimate mail.
...
> **Quick answer:** I rolled DMARC to p=reject across three personal domains without losing legit mail. Step-by-step SPF, DKIM, DMARC, and the failure modes nobody warns about.
>
...
DMARC tells receiving mail servers what to do when an incoming message claims to be from your domain but fails SPF or DKIM checks. With p=reject, the receiver drops the message. With p=none, the receiver only reports it. Reject is the goal — none is the starting line.
...
## The order I deployed in

SPF first, then DKIM, then DMARC at p=none, then forensic reports for two weeks, then p=quarantine for two weeks, then p=reject. Skipping any step is how legitimate mail starts disappearing.
...
v=spf1 include:_spf.protonmail.ch include:_spf.fastmail.com -all. The hard fail (-all) is intentional. A soft fail (~all) defeats the point of locking your domain.
...
## DKIM signing through Proton and Fastmail

Both providers gave me three CNAME records to add to my DNS. Within 48 hours of adding them, I saw DKIM=pass on every outbound message in the DMARC reports.
...
I forwarded reports to a dedicated Mailbox.org address and parsed them with parsedmarc. Within ten days I had a clear picture of every legitimate sender on my domains and every spoofing attempt.
...
## Handling forwarded mail and mailing lists

Forwarding breaks SPF. Mailing lists break DKIM unless they use ARC. I added ARC-aware forwarding via SimpleLogin and switched my old Gmail forwards to a Proton Catch-All.
...
## The week I spent at p=quarantine

Spam folders filled up with a few legitimate messages from a CRM I had forgotten. I added it to SPF, waited 24 hours, and the mail flowed again. Without quarantine I would have rejected those silently.
...
## Final state and ongoing maintenance

All three domains now sit at p=reject with rua reports going to a single inbox. I review them monthly, takes about 15 minutes. Spoofing attempts dropped to functionally zero.
